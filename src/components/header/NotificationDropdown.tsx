...
export default function NotificationDropdown() {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      if (!user?.sub) return; // Only fetch if we have a user ID
      
      try {
        const userNotifications = await notificationService.getUserNotifications(user.sub);
        setNotifications(userNotifications);
        setNotifying(userNotifications.some(n => 
          !n.read.some(r => r.userId === user.sub)
        ));
      } catch (error) {
    ...
    try {
      ...
      setNotifications(notifications.map(n => 
        n._id === notificationId 
          ? { ...n, read: [...n.read, { userId, readAt: new Date() }] }
          : n
      ));
    } catch (error) {
